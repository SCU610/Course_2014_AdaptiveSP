%Two-weight Lattice adaptive structure implementation using SER algorithm
%***************************************************************************
%****      Two-weight Lattice adaptive structure using SER algorithm    ****
...
StepLength=0.02;%SER iteration step
IterationCount=0;
...
while IterationCount<1000 
    ...
    if IterationCount==0
        P(:,IterationCount+1)=[mse(Xk);mse(ErrorForw1(:,IterationCount+1))];
        W(:,IterationCount+1+1)=W(:,IterationCount+1);
    elseif IterationCount>=1
        P(:,IterationCount+1)=((1-Alpha).*[mse(Xk);mse(ErrorForw1(:,IterationCount+1))]+Alpha.*(1-Alpha.^(IterationCount)).*P(:,IterationCount))/(1-Alpha.^(IterationCount+1));
        W(:,IterationCount+1+1)=W(:,IterationCount+1)-StepLength.*[ErrorForw1(IterationCount+1,IterationCount+1).*Xk(IterationCount,1)/P(1,IterationCount+1);ErrorForwGlobal(IterationCount+1,IterationCount+1).*ErrorBack1(IterationCount,IterationCount)/P(2,IterationCount+1)];
    else
        break;
    end
